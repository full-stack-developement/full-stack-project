...
export function schemaValidationMiddleware(schema: AnyObject) {
  return async function (req: ISchemaRequest, res: Response, next: NextFunction) {
    try {
      const data = req.body;
      const schemaValidated = await schema.validate(data, { abortEarly: false, stripUnknown: true });
