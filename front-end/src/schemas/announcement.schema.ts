...
export const announcementUpdateSchema = yup.object().shape({
    ...
    year : yup.number().optional().test({message : "Digite um ano válido",test : (value)=>{
        if(value){
                const valueString = String(value)
            const year = new Date().getFullYear()
            if(valueString.length < 4 || valueString.length > 4){
    ...
    km : yup.number().optional().transform((value) => (isNaN(value) || value === null || value === undefined) ? 0 : value),
    price : yup.number().optional().transform((value) => (isNaN(value) || value === null || value === undefined) ? 0 : value),
